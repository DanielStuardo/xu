...
if numParam>1
   for i:=1 to len(_arr_par)
      ...
      elseif STRING=="-bin"   // carga binario automaticamente
         SWBINARY:=.T.
...
ytemp:=ROW()
   ...
   CABECERA:=" Edit4XU"+substr(CABECERA,9)
...
//              end
     ...
     if SWPRIMERAVEZ  // marca de palabra match ^NN
        if TEXTOTIPO=="BINARY"
           if px+len(cBUSCA) > nOFFSET+1+20
              c2:=(px+len(cBUSCA)) - (nOFFSET+20)
              COLORWIN(py, px, py, nOFFSET+20,32) //px+(len(cBUSCA)-c2),32 )
              COLORWIN(py+1, nOFFSET+1, py+1, nOFFSET+c2-1,32 )
              c2:=((23+len(cBUSCA)*3)+(px-nOFFSET-1)*3-2) - (nOFFSET-4)
     ...
     if c==1001 .and. TEXTOTIPO!="BINARY"
        ...
        if c==1002     // izquierdo
           ...
           if c==1003 
              mcCol:=MCOL()
              mcRow:=MROW()
             // @ 0,40 say "IZQ->"+alltrim(str(MROW()))+" - "+alltrim(str(MCOL()))+" P="+hb_ntos(p)+" PI="+hb_ntos(pi)
            ...
            /*     if mcCol>=11 .and. mcCol<=23  // help
             ...
             /* elseif mcRow==TLINEA
              ...
              //@ 0,40 say "IZQ->"+alltrim(str(MROW()))+" - "+alltrim(str(MCOL()))+" P="+hb_ntos(p)
              setpos(py,px)
           end
      ...
      //  else
      //     @ 0,40 say "CENTRO->"+alltrim(str(MROW()))+" - "+alltrim(str(MCOL()))
        end
        c:=0
        while inkey(,159)!=0 ; end
        MRESTSTATE(MOUSE)
       ...
       if STRING=="*" .or. SW_MODIFICADO
          if !SW_CTRLOF
             cMessage := "Tienes archivos que no han sido guardados" + _CR + ;
                            ...
                            "Deseas salir de Ed4XU de todas maneras?"
                 ...
                 if cLEN>0
                    if AT(",",BUFFER[1])>0  // hay lista separada por comas. PUede ser matriz
                     ...
                     //  SW_PASTE:=.T.
                       ...
                       TEXTO[pi]:=substr(TEXTO[pi],1,p-1)+alltrim(str(nSol))+substr(TEXTO[pi],p,len(TEXTO[pi]))
                    end
                 end
                 exit
              elseif nChoice==5
...
*/
     ...
     elseif c==11   //CTRL-K   // borra una linea, pero la deja en el buffer
       ...
       MSGCONTROL(" CTRL-K... K|F3=Cut line | U|F4=Paste line | G=Del Segment | X=Del Lines  | I=Pos Ini line |  P=Indent line",;
                  " EDIT      E=Del to EOL  | C=Copy Sqr Blk  | S=Sangria(+)  | D=Sangria(-) | N=AutoComplete | ^P=Indent Lines",;
                  "           B=Del to BOL  | M=Cut Sqr Blk   | Q=Cancel Mark | L=Del BUFFER | V=View BUFFER  | More in ^O+O...")
       c:=inkey(0)
       c:=asc(upper(chr(c)))
       BarraTitulo(ARCHIVO)
       setpos(py,px)
...
//         end
           ...
           /*  if EXT=="xu" .or. EXT=="def"
             ...
             for i:=1 to LENMETA
                 ...
                 if METADATA[i]==ARCHIVO   //tARCHIVO
                    ...
                    STRING:=" "+alltrim(str(XLEN))+padl(STRING,40)+" (editing) ("+PATH+")"
                 else
                    if VERIFICAMODIFICADO(STRING,cMETADATA)
                       STRING:=" "+alltrim(str(XLEN))+padl(STRING,40)+" (mod)     ("+PATH+")"
                    else
                       STRING:=" "+alltrim(str(XLEN))+padl(STRING,40)+"           ("+PATH+")"
                    end
                 end
                 ++XLEN
     ...
     elseif c==15    // CTRL-O   HELP
       if TEXTOTIPO=="BINARY"
       MSGCONTROL(" CTRL-O... O=Help Ed4Xu | F=Open File | A=Details",;
     hb_utf8tostr(" HELP      X=Help XU    | N=New file    "),;
                  " & FILE   F2=Open File")
       else
       MSGCONTROL(" CTRL-O... O=Help Ed4Xu  |  F=Open File |            | P=Repeat last OPE command",;
     hb_utf8tostr(" HELP      X=Help XU     |  N=New file  | A=Details  | S=Execute Shell Command"),;
                  " & FILE                  | F2=Open File |            | L=Search & Load Program from current directory")
       end
       c:=inkey(0)
       c:=asc(upper(chr(c)))
       BarraTitulo(ARCHIVO)
       ...
       elseif c==67   // CTRL-OC  busca ayuda de la función XU donde está parado
          
       elseif c==65   // CTRL-OA   detalles del acerca de
          cMessage := hb_utf8tostr("                   EDIT4XU v.1.0                   "+_CR+;
                      "                                                   "+_CR+;
...
*/
             ...
             if len(COMANDO)>0
                ...
                ELSEIF SWREPLACECODIGO
                   ...
                   BUFFER_CTRLZ:={};LINBUFFCTRLZ:={}
                      NL:=numtoken(OUTPUTCOMM,HB_OSNEWLINE())
                      NUMCAR:=len(OUTPUTCOMM)
                      MAXLONG:=3200
       ...
       elseif c== 79  //.or. c==15    // CTRL-OO  Ayuda del editor
          HELPTOTAL:=CARGAINTROXU() ///hb_utf8tostr(MEMOREAD(PATH_HELP+_fileSeparator+"ed4xu.help"))
          MSGTOTAL:="AYUDA DE ED4XU          ^C=Av Pag.  ^R=Re Pag. "
          hb_keyPut(27)
       elseif c==1
       ...
       elseif c==79   // ayuda Ed4xu
          CALLED4XUMENU(@HELPTOTAL,ARCHIVO)
       else
        ...
        //if c>=32
          ...
          if c!=0
             ...
             ///     AADD(CTRLZ,{lini,cini,pi,p,py,px,TEXTO[pi]})
                ...
                TEXTO[pi]:=LLENATEXTO(s,len(s),0)
               // @ py+1, px say TEXTO[pi] + alltrim(str(len(TEXTO[pi]))) ; inkey(0)
                setcursor(0)
                VISUALIZA_TEXTO(TEXTO,lini,lfin,cini,SLINEA,TOPE,XFIL1EDIT,XFIL2EDIT,XFILi)
...
FUNCTION CALLED4XUMENU(HELPTOTAL,ARCHIVO)
...
while nChoice>0 
  ...
  // end
   ...
   if nChoice==1
      ...
      MATCH:={"Introduccion ED4XU","Uso del BUFFER","Comandos de movimiento de cursor","Edicion basica",;
              "Menu ^O (ayuda y archivos)","Ayuda ^P (Compilacion y vista)","Menu ^V (Insercion de codigo)",;
              "Menu ^J (Saltos a lineas)",;
              "Menu ^K (Edicion avanzada)","Menu ^N (Busqueda y reemplazo)","Menu ^L (Procesamiento avanzado de texto)"}
      FILEHELP:=PATH_HELP+_fileSeparator+"ed4xu.help"
      XLEN:=len(MATCH)

   elseif nChoice==2
      TOPICO:="MACROS - EDICION AVANZADA"
      MATCH:={"Introduccion a las macros ED4XU - OPE","Sentencias del encabezado","Registros de memoria","Estructura de Ciclo",;
              ...
              "Funciones de archivo","Operadores de bits","Edicion hexadecimal","Ejemplos","Comandos ALT (atajos de comandos CTRL)"}
      FILEHELP:=PATH_HELP+_fileSeparator+"ed4xu.help"
      XLEN:=len(MATCH)
...
fecha:=date()
sFec:=meses[MONTH(fecha)]+" "+alltrim(str(DAY(fecha)))+", "+alltrim(str(YEAR(fecha)))
...
if len(alltrim(DX))==0
   for i:=1 to cLEN
      AADD(BUFFER,alltrim(str(nPOS[i]))+chr(127) )
   end
else
   ...
   if AX=="L"
      for i:=1 to cLEN
         AADD(BUFFER,padl(alltrim(str(nPOS[i])),CX,BX)+chr(127) )
      end
   elseif AX=="R"
      for i:=1 to cLEN
         AADD(BUFFER,padr(alltrim(str(nPOS[i])),CX,BX)+chr(127) )
      end
   else
      for i:=1 to cLEN
         AADD(BUFFER,padc(alltrim(str(nPOS[i])),CX,BX)+chr(127) )
      end
   end
end
RETURN
...
//   ? "ENTRA: ",cDATO; inkey(0)
   RX:=_EVALUA_EXPR(@R,cDATO,I,@BUFFER)
   if valtype(RX)=="L"
      DEFTOKEN:=TMPTOKEN
      RETURN .F.
   end
...
DICC:={"LN","LOG","SQRT","ABS","INT","CEIL","FLOOR","SGN","ROUND","SIN","COS","TAN","UP","LO","MSK","MON","SAT","SAVE","LOAD",;
       ...
       "JNZ","ELSE","ENDIF","CLEAR","RANGE","STR",;
...
for i:=1 to cLEN
   ...
   BUFFER[i]:=alltrim(str(abs(nDATO)))+chr(127)
end
RETURN
...
for i:=1 to cLEN
   ...
   BUFFER[i]:=alltrim(str(log(nDATO)))+chr(127)
end
RETURN
...
for i:=1 to cLEN
   ...
   BUFFER[i]:=alltrim(str(log10(nDATO)))+chr(127)
end
RETURN
...
for i:=1 to cLEN
   ...
   BUFFER[i]:=alltrim(str(nDATO^2))+chr(127)
end
RETURN
...
for i:=1 to cLEN
   ...
   BUFFER[i]:=alltrim(str(sqrt(nDATO)))+chr(127)
end
RETURN
...
if nChoice==1 .or. nChoice==3  // vertical o total
   ...
   if nChoice==1   // vertical
      if mainChoice==1
         for i:=1 to nOCURR-1
            tBUFFER[1]+=alltrim(str(aSUM[i]))+TOK
         end
         tBUFFER[1]+=alltrim(str(aSUM[nOCURR]))
      elseif mainChoice==2
         for i:=1 to nOCURR-1
            tBUFFER[1]+=alltrim(str(aSUM[i]/cLEN))+TOK
         end
         tBUFFER[1]+=alltrim(str(aSUM[nOCURR]/cLEN))
      elseif mainChoice==3  // std
         for i:=1 to nOCURR  // para cada columna
            ...
            tBUFFER[1]+=alltrim(str( SQRT(nSUM/cLEN) ))
            if i<nOCURR
               tBUFFER[1]+=TOK
            end
         end
         ...
         for i:=1 to nOCURR  // para cada columna
            ...
            tBUFFER[1]+=alltrim(str( (nSUM/cLEN) ))
            if i<nOCURR
               tBUFFER[1]+=TOK
            end
         end
      ...
      if mainChoice==1
         ...
         tBUFFER[1]:=alltrim(str(nSUM))
      elseif mainChoice==2
         for i:=1 to nOCURR
            nSUM+=aSUM[i]
         end
         tBUFFER[1]:=alltrim(str(nSUM/(nOCURR*cLEN)))
      elseif mainChoice==3  // std
         for i:=1 to nOCURR
            nSUM+=aSUM[i]
         end
         ...
         tBUFFER[1]:=alltrim(str( SQRT(nSUM/(cLEN*nOCURR)) ))
      else    // var
         for i:=1 to nOCURR
            nSUM+=aSUM[i]
         end
         ...
         tBUFFER[1]:=alltrim(str( (nSUM/(cLEN*nOCURR)) ))
      end
   end
...
elseif nChoice==2   // horizontal
   ...
   for i:=1 to cLEN
      ...
      nPROM:=nSUM/nOCURR
      if mainChoice==1
         tBUFFER[i]:=alltrim(str(nSUM))+chr(127)
      elseif mainChoice==2
         tBUFFER[i]:=alltrim(str(nPROM))+chr(127)
      elseif mainChoice==3  // std
         nSUM:=0
         for j:=1 to nOCURR
            nSUM:=nSUM + ((nPOS[j]-nPROM)^2)
         end
         tBUFFER[i]:=alltrim(str(SQRT(nSUM/nOCURR)))+chr(127)
      else   // var
         nSUM:=0
         for j:=1 to nOCURR
            nSUM:=nSUM + ((nPOS[j]-nPROM)^2)
         end
         tBUFFER[i]:=alltrim(str(nSUM/nOCURR))+chr(127)
      end
      RELEASE nPOS
      nSUM:=0
   end
...
//
    ...
    if TIPO==2
       CX:=MEMOREAD(PATH_LOG+_fileSeparator+"XUANS_"+EX+".log")
       RX:=iif(CX!="0",.F.,.T.)
...
if !FILE(PATH_XU+_fileSeparator+"ed4xu.compiler")
   DEFINEKEYWORDS(LANGUAJE)
else
   _v:=Memoread(PATH_XU+_fileSeparator+"ed4xu.compiler")
   _nl:= MlCount(_v)
...
if !FILE(PATH_XU+_fileSeparator+"ed4xu.compiler")
   outstd(_CR,hb_UTF8tostr("Atención: no encuentro el archivo ED4XU.COMPILER"),_CR)
...
/*HB_FUNC( PONECOLORALAHUEA )
{
   ...
   const char * pSTRING = hb_itemGetC( tSTRING );

   long uiArrayLen = ( long ) hb_arrayLen( pListaK );
   for(i=1; i<=uiArrayLen;i++){  // para cada elemento de la lista
      //const char * pElemento = hb_itemGetCPtr( hb_itemArrayGet( pListaK, i) );
      const char * pElemento = hb_itemGetC( hb_itemArrayGet( pListaK, i) );
   ...
   for(i=1; i<=uiArrayLen;i++){  // para cada elemento de la lista
      //const char * pElemento = hb_itemGetCPtr( hb_itemArrayGet( pListaS, i) );
      const char * pElemento = hb_itemGetC( hb_itemArrayGet( pListaS, i) );
      long sLen = strlen( pElemento );
   ...
   for(i=1; i<=uiArrayLen;i++){  // para cada elemento de la lista
      //const char * pElemento = hb_itemGetCPtr( hb_itemArrayGet( pListaD, i) );
      const char * pElemento = hb_itemGetC( hb_itemArrayGet( pListaD, i) );
      long sLen = strlen( pElemento );
